"""
pytest_auditor — CLI entry point and pytest plugin.
All .py files must be in the same directory as this file.
...
def _load(name: str):
    """Load a sibling module, registering it in sys.modules so dataclasses work."""
    full_name = f"_pytest_auditor_{name}"
    if full_name in sys.modules:
        return sys.modules[full_name]
    spec = importlib.util.spec_from_file_location(full_name, HERE / f"{name}.py")
    mod  = importlib.util.module_from_spec(spec)
    sys.modules[full_name] = mod   # register BEFORE exec so cross-refs resolve
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="pytest-auditor",
        description="Audit a pytest test suite — 62 checks, terminal + HTML report",
    )
...
def pytest_sessionfinish(session, exitstatus):
    ...
    report = analyse.scan(tests_root)
    terminal.console.print("\n")
    terminal.print_report(report)

    html_path = Path(session.config.getoption("--report-html"))
    html_rep.generate_html(report, html_path)
    terminal.console.print(f"  [dim]HTML → [bold]{html_path}[/bold][/dim]\n")
    ...
    if fail_under and report.score < fail_under:
        terminal.console.print(
            f"[bold red]Score {report.score} < {fail_under} — failing[/bold red]"
        )
